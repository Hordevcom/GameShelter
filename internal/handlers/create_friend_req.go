...
type FriendReqAdder interface {
	CreateFriendReqService(ctx context.Context, payload models.FriendRequestJSON, token string) error
}

func CreateFriendReqHandler(logger logging.Logger, fra FriendReqAdder) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = fra.CreateFriendReqService(r.Context(), UserFrendsReq, cookie.Value)
		if err != nil {
		...
		render.JSON(w, r, map[string]string{"message": "Request has created!"})
	}
}
