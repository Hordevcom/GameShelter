...
type FriendReqRemover interface {
	DeleteFriendReqService(ctx context.Context, payload models.FriendRequestJSON, token string) error
}

func DeleteFriendReqHandler(logger logging.Logger, frr FriendReqRemover) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = frr.DeleteFriendReqService(r.Context(), UserFrendsReq, cookie.Value)
		if err != nil {
		...
		render.JSON(w, r, map[string]string{"message": "Request was deleted!"})
	}
}
