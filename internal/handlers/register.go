...
type UserAdderService interface {
	AddUserToDB(ctx context.Context, user models.UserAuth) error
}

func UserRegister(userAuth UserAuth, logger logging.Logger, userAdder UserAdderService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		loginExist := userAuth.CheckUserLoginService(r.Context(), user)
		...
		err = userAdder.AddUserToDB(r.Context(), user)
		if err != nil {
			logger.Error("problem with add user to db: ", err)
